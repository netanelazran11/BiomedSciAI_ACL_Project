...
class DNASeqCorePromoterDataset(BaseDNASeqDataset):
    DATASET_NAME = "core_promoter"
    default_label_dict_path = (
        Path(__file__).parent / f"{DATASET_NAME.lower()}_all_labels.json"
    )


class DNASeqCorePromoterDataModule(DNASeqDataModule):
    DATASET_FACTORY: Dataset = DNASeqCorePromoterDataset
