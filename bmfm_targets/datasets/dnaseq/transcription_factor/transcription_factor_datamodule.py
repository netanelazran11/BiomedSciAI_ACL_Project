...
class DNASeqTranscriptionFactorDataset(BaseDNASeqDataset):
    DATASET_NAME = "transcription_factor"
    default_label_dict_path = (
        Path(__file__).parent / f"{DATASET_NAME.lower()}_all_labels.json"
    )


class DNASeqTranscriptionFactorDataModule(DNASeqDataModule):
    DATASET_FACTORY: Dataset = DNASeqTranscriptionFactorDataset
