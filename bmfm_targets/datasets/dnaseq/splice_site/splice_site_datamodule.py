...
class DNASeqSpliceSiteDataset(BaseDNASeqDataset):
    DATASET_NAME = "splice_site"
    default_label_dict_path = (
        Path(__file__).parent / f"{DATASET_NAME.lower()}_all_labels.json"
    )


class DNASeqSpliceSiteDataModule(DNASeqDataModule):
    DATASET_FACTORY: Dataset = DNASeqSpliceSiteDataset
