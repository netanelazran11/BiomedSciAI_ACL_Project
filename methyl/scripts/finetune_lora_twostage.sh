...
# Two-stage: head warms up for N epochs before LoRA unlocks
# Typical frozen convergence is ~30-50 epochs; set slightly above to be safe
LORA_WARMUP_EPOCHS="${LORA_WARMUP_EPOCHS:-50}"

# Training hyperparameters
LEARNING_RATE="${LEARNING_RATE:-1e-3}"   # Head LR (unchanged in both stages)
BATCH_SIZE="${BATCH_SIZE:-16}"
ACCUMULATE_GRAD="${ACCUMULATE_GRAD:-4}"  # Effective batch = 16 * 4 = 64
FINETUNE_EPOCHS="${FINETUNE_EPOCHS:-300}"
EARLY_STOP_PATIENCE="${EARLY_STOP_PATIENCE:-100}"  # Extra patience for LoRA stage transition
HEAD_DROPOUT="${HEAD_DROPOUT:-0.1}"

# W&B naming
WANDB_ENTITY="netanelazran11-hebrew-university-of-jerusalem"
WANDB_PROJECT="finetune-lora-twostage"
WANDB_RUN_NAME="twostage-r${LORA_RANK}-w${LORA_WARMUP_EPOCHS}-k${SUBSET_K}-${SLURM_JOB_ID}"
...
echo "============================================================"
echo "TWO-STAGE LoRA FINE-TUNING"
echo "============================================================"
...
echo "Stage 1:     epochs 0-$((LORA_WARMUP_EPOCHS-1))  → head only (LoRA LR=0)"
echo "Stage 2:     epoch ${LORA_WARMUP_EPOCHS}+         → LoRA unlocked (lr=${LORA_LR})"
echo "Head LR:     ${LEARNING_RATE} (constant through both stages)"
...
# -------------------------
python -m bmfm_methylation.finetune_lora_twostage \
    data_path="${DATA}" \
    ...
    lora_lr=${LORA_LR} \
    lora_warmup_epochs=${LORA_WARMUP_EPOCHS} \
...
echo "============================================================"
echo "Two-Stage LoRA Fine-tuning finished: $(date)"
echo "============================================================"
