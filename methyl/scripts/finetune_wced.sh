...
echo "CpG Subset:  FIXED ${SUBSET_K} CpGs (seed=${FIXED_SUBSET_SEED})"
echo "Pooling:     CLS (pooler_output) — WCED-trained global representation"
echo "Head LR:     ${LEARNING_RATE}  |  Encoder LR: $(python3 -c "print(${LEARNING_RATE} * 0.1)")"
echo "Batch:       ${BATCH_SIZE} x ${ACCUMULATE_GRAD} = $((BATCH_SIZE * ACCUMULATE_GRAD)) effective"
echo "Freeze:      ${FREEZE_EPOCHS} epochs then unfreeze"
