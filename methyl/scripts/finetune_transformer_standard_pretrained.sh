#!/bin/bash -l
#SBATCH --job-name=finetune-standard-ckpt-8k
...
# Standard pretrained checkpoint (trained WITH CpG IDs, loss=0.0149)
# This checkpoint has well-trained transformer layers, beta embeddings, and position embeddings.
# The value-only finetune script will use beta + position embeddings only (skip CpG IDs).
...
WANDB_RUN_NAME="finetune-standard-ckpt-${SLURM_JOB_ID}"
...
echo "============================================================"
echo "MODE:        STANDARD PRETRAINED (CpG IDs used in pretrain, skipped in finetune)"
...
# -------------------------
# Run Value-Only Transformer Fine-tuning (FROM STANDARD PRETRAINED)
# Uses beta_embed + pos_embed + transformer layers from standard checkpoint.
# CpG ID embeddings are loaded but NOT used in the forward pass.
# -------------------------
