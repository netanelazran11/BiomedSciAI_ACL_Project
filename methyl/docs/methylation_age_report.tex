...
\textbf{Metrics:} We report MAE (interpretable in years), MedAE (robust to outliers), and $R^2$ (proportion of variance explained). These are standard metrics for age regression that allow comparison with prior work~\cite{horvath,hannum}.
...
We adapt BMFM-RNA~\cite{bmfm}, a BERT-based foundation model for RNA-seq ($\sim$110M parameters), to methylation data.
...
\item \label{methylgpt} Ying, K., et al., ``MethylGPT: a foundation model for the DNA methylome,'' \emph{bioRxiv}, 2024.
\item \label{bmfm} Dandala, B., et al., ``BMFM-RNA: An Open Framework for Building Transcriptomic Foundation Models,'' \emph{arXiv}, 2025.
\item \label{horvath} Horvath, S., ``DNA methylation age of human tissues and cell types,'' \emph{Genome Biology}, 14(10):R115, 2013.
\item \label{hannum} Hannum, G., et al., ``Genome-wide methylation profiles reveal quantitative views of human aging rates,'' \emph{Molecular Cell}, 49(2):359--367, 2013.
