#!/usr/bin/env python3
"""
LoRA Fine-tuning for Methylation Age Prediction — WCED pretrained encoder.

Uses LoRA (Low-Rank Adaptation) to adapt the pretrained encoder without
catastrophic forgetting. Original weights stay frozen; only small rank-8
adapter matrices (A, B) train in the attention layers.

Why LoRA instead of full unfreeze:
  - Full unfreeze → only age gradients → encoder forgets global representation
  - LoRA → 0.4% of encoder params change → cannot diverge from pretrained CLS
  - Frozen weights are a mathematical guarantee of preservation (vs decoder regularizer)

Why 8k fixed input (not 4k random):
  - No decoder → no need for partial input → use ALL 8k CpGs every batch
  - Maximum information per step → better age prediction
  - Fixed subset → stable gradients → faster convergence

Pipeline:
    8k fixed CpGs → WCED Encoder (frozen) + LoRA adapters → CLS → MLP → age

LoRA targets: query and value projections in all 6 attention layers
LoRA params:  ~98k  (0.4% of encoder)
Head params:  ~165k (always fully trained)
"""
...
from bmfm_methylation.lora import inject_lora, get_lora_parameters

logger = logging.getLogger(__name__)


class MethylationAgeRegressorLoRA(pl.LightningModule):
    """
    LoRA fine-tuning module for methylation age regression.

    Encoder: WCED pretrained, frozen except LoRA adapters in query+value
    Head:    MLP fully trained at higher LR
    Input:   8k fixed CpGs every batch (maximum information)
    """
    ...
    def __init__(
        ...
        logger.info(f"LoRA Fine-tuning module:")
        logger.info(f"  Encoder frozen:    {frozen_params:,}")
        logger.info(f"  Encoder LoRA:      {lora_params:,}  (trainable)")
        logger.info(f"  Age head:          {head_params:,}  (trainable)")
        logger.info(f"  Head LR:           {learning_rate}  |  LoRA LR: {lora_lr}")
    ...
    def configure_optimizers(self):
        ...
        # Age head — fast learning (lr=1e-3)
        ...
        # LoRA adapters — medium learning (lora_lr=1e-4)
        ...
        optimizer_grouped_parameters = [
            ...
            {"params": lora_params,          "lr": self.hparams.lora_lr,
             "weight_decay": 0.0},   # no decay on LoRA adapters
        ]
        optimizer_grouped_parameters = [
...
def setup_wandb(cfg: DictConfig):
    if hasattr(cfg, 'track_wandb') and cfg.track_wandb.get('enabled', False):
        try:
            ...
            return WandbLogger(
                project=cfg.track_wandb.get('project', 'methylation-age-lora'),
    ...
    return TensorBoardLogger(cfg.output_directory, name="finetune_lora")
...
def main(cfg: DictConfig):
    logger.info("=" * 70)
    logger.info("METHYLATION AGE FINE-TUNING — LoRA (encoder frozen + adapters)")
    logger.info("=" * 70)
    ...
    data_module = MethylationDataModule(
        ...
        subset_k=subset_k,
        fixed_subset=True,              # fixed 8k — stable gradients, max info
        fixed_subset_seed=cfg.data_module.get('fixed_subset_seed', 42),
        use_wced_collator=False,        # no decoder → no need for all_betas
    )
    data_module.setup()
    ...
    # -------------------------------------------------------------------------
    # Inject LoRA into encoder attention layers
    # -------------------------------------------------------------------------
    ...
    logger.info(f"LoRA injected: rank={lora_rank}, alpha={lora_alpha}, lr={lora_lr}")
    ...
    model = MethylationAgeRegressorLoRA(
        encoder=encoder,
    ...
    callbacks = [
        pl.callbacks.ModelCheckpoint(
            dirpath=output_dir / "finetune_lora" / "checkpoints",
    ...
    trainer = pl.Trainer(
        ...
        logger=wandb_logger,
        callbacks=callbacks,
        default_root_dir=str(output_dir / "finetune_lora"),
    ...
    logger.info(f"LoRA fine-tuning complete. Best checkpoint: {best_ckpt}")
